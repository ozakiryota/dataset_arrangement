...
class FileListRowsMerge:
    ...
    def exec(self):
        ...
        parent_file_path_list_list += child_file_path_list_list

        if self.args.write_dir_path == None:
            self.args.write_dir_path = pathlib.Path(os.path.dirname(self.args.read_parent_csv_path))        
        for i, file_path_list in enumerate(parent_file_path_list_list):
            parent_file_path_list_list[i] = [file_path.relative_to(self.args.write_dir_path) for file_path in file_path_list]
...
if __name__ == '__main__':
    file_list_rows_merge = FileListRowsMerge()
    file_list_rows_merge.exec()